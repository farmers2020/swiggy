...
def update_city(request):
    ...
    sname = request.GET.get("cname")
    d1 = {"sno": cno, "cname": sname}
...
def update_type_data(request):
    cno = request.POST.get("s1")
    cname = request.POST.get("s2")
    RestaurantTypeModel.objects.filter(no=cno).update(type_name=cname)
    return redirect('open_type')
